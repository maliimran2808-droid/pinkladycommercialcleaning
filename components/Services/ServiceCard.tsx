...
export default function ServiceCard({ data }: ServiceCardProps) {
  return (
    <div className="bg-white cursor-pointer px-6 py-7.5 rounded-4xl transition-all duration-500 transform hover:-translate-y-2 flex flex-col h-full">
      
      {/* Top: Icon & Title */}
      <div className="flex items-start pb-4 flex-col gap-4">
        <div 
          className="w-21 h-21 rounded-[25px] flex items-center justify-center flex-shrink-0"
          style={{ backgroundColor: '#fadced' }} // Light violet background
        >
          {data.icon}
        </div>
        ...
        {data.features.map((feature, index) => (
          <li key={index} className="flex items-center mb-2.5 gap-2">
            <div className="w-[fit-content] flex items-center justify-start flex-shrink-0">
              <Check size={13} className="text-[#E10788]" strokeWidth={3} />
            </div>
      ...
      <Link
        href={data.link}
        className="font-parkinsans flex items-center justify-between w-full text-sm px-5 py-2.5 rounded-full cursor-pointer text-white font-semibold tracking-normal transition-all duration-300 hover:shadow-lg transform hover:scale-[1.0]"
        ...
        <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center transition-transform duration-300 group-hover:translate-x-1">
          <ArrowRight size={16} className="text-[#E10788]" />
        </div>
      </Link>
    </div>
  )
